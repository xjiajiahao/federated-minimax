...
from .transformed_tensor_dataset import TransformedTensorDataset
import warnings


class FastFashionMNIST(torchvision.datasets.FashionMNIST):
    """The (multiclass) FashionMNIST dataset (fast version).
...
class FashionMNISTLoader(object):
    """The multiclass or binary FashionMNIST dataset loader.

    The original classes = (0, 1, ..., 9).
    ...
    Arguments:
        ...
        image_size (int, default: None): the desired image size, if image_size is None, the original size will be used
    """


    def __init__(self, num_partitions, data_root='./data/', train_batch_size=32, test_batch_size=32, is_multiclass=False, pos_classes=[0], device='cpu', sort_by=None, similarity=0, image_size=None):
        if is_multiclass:
        ...
        if self.image_size is not None:
            ...
            self.feature_shape = [1, image_size, image_size]
        else:
            self.transform = None
            self.feature_shape = [1, 28, 28]

        self.train_set = FastFashionMNIST(data_root=data_root,
                                            train=True,
    ...
    def build_test_loader(self):
        if self.test_set is None:
            self.test_set = FastFashionMNIST(data_root=self.data_root,
                                              train=False,
                                            #   train=True,
                                              transform=self.transform,
                                              download=True,
...
def BuildFedFashionMNISTDataset(num_partitions, data_root='./data/', sort_by=None, similarity=0):
    """Constructing the federated FashionMNIST dataset.
    ...
    Arguments:
        ...
        similarity (float number in the range [0, 1]): the ratio of random partitioned data (default: 0, no similarity), only applicable to sort_by='label'
    """
    train_set = FastFashionMNIST(data_root=data_root,
                                   train=True,
                                   download=True,
                                   device='cpu',
                                   sort_by=sort_by,
    ...
    test_set = FastFashionMNIST(data_root=data_root,
                                          train=False,
                                        #   train=True,
                                          download=True,
                                          device='cpu')
    ...
    # save data files
    data_folder_name = 'fed_fashion_mnist_n_' + str(num_partitions) + '_sort_by_' + sort_by + '_sim_' + str(similarity)
...
class FedFashionMNISTLoader(object):
    """The federated FashionMNIST dataset loader.

    The original classes = (0, ..., 9).
    ...
    def __init__(self, num_partitions, data_root='./data/', train_batch_size=32, test_batch_size=32, is_multiclass=False, pos_classes=[0], sort_by=None, similarity=0, device='cpu', image_size=None):
        self.pos_ratio = len(pos_classes) / 10
        self.train_batch_size = train_batch_size
        self.test_batch_size = test_batch_size
        self.is_multiclass = is_multiclass
    ...
    def build_train_loader(self, partition_idx):
        ...
        if self.image_size is not None:
            train_set = TransformedTensorDataset(features, labels, transform=self.transform)
        else:
    ...
    def build_test_loader(self):
        ...
        features, labels = features.to(self.device), labels.to(self.device)
        if self.image_size is not None:
            test_set = TransformedTensorDataset(features, labels, transform=self.transform)
        else:
